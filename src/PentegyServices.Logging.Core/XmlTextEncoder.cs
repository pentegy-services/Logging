...
namespace PentegyServices.Logging.Core
{
	...
	{
		/// <summary>Helper method for fast string encoding.</summary>
		...
		public XmlTextEncoder(TextReader source, Boolean filterIllegalChars = true)
		{
			_source = source;
		...
		readonly Boolean _filterIllegalChars;
			...
			while (_buf.Count == 0 && _source.Peek() != endSentinel)
			{
				...
				else if (!_filterIllegalChars)
				{
					...
					throw new ArgumentException(
						String.Format("Illegal character: '{0:X}'", (Int32)c));
				}
			}
		}
